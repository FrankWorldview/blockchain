...
NFTs typically use metadata to describe their visual or functional properties — including name, image, and custom traits. In ERC-721, this is accessed through the `tokenURI(uint256 tokenId)` function, which returns a URI that points to a JSON metadata file.
...
OpenZeppelin’s `ERC721URIStorage` is an extension of the standard ERC-721 contract. It allows developers to store a **unique metadata URI** for each token **directly on-chain**, using built-in storage mappings and helper functions.

---

### 🔁 Relationship Between `ERC721URIStorage`, `_setTokenURI()`, and `tokenURI()`
...
> Think of `_setTokenURI()` as the **“write”** operation, and `tokenURI()` as the **“read”** operation.
...
|-----------------------|-------------------------------------------------------|------------------|------------------------------------------|
| `tokenURI()` (custom) | Manually override to generate or return off-chain URI | ❌               | Dynamic metadata (IPFS, HTTPS)           |
| `ERC721URIStorage`    | Store a unique URI on-chain using `_setTokenURI()`    | ✅               | Static, verifiable metadata per token    |
